...
export class DphControllerBase {
  constructor(
  ...
  @swagger.ApiCreatedResponse({ type: Dph })
  @swagger.ApiForbiddenResponse({ type: errors.ForbiddenException })
  async create(
    @common.Body() data: DphCreateInput,
  ...
  @swagger.ApiOkResponse({ type: [Dph] })
  @swagger.ApiForbiddenResponse()
  @swagger.ApiQuery({
    type: () => DphFindManyArgs,
    style: "deepObject",
    explode: true,
  })
  async findMany(
    ...
    const args = plainToClass(DphFindManyArgs, request.query);
  ...
  @swagger.ApiForbiddenResponse({ type: errors.ForbiddenException })
  async findOne(
    @common.Param() params: DphWhereUniqueInput,
    @nestAccessControl.UserRoles() userRoles: string[]
  ): Promise<Dph | null> {
    const permission = this.rolesBuilder.permission({
      role: userRoles,
  ...
  async createOrders(
    @common.Param() params: DphWhereUniqueInput,
    @common.Body() body: DphWhereUniqueInput[],
    @nestAccessControl.UserRoles() userRoles: string[]
  ): Promise<void> {
    const data = {
      orders: {
    ...
    if (invalidAttributes.length) {
      ...
      throw new common.ForbiddenException(
        `Updating the relationship: ${
          invalidAttributes[0]
        } of ${"Dph"} is forbidden for roles: ${roles}`
      );
    }
